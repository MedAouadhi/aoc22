...
impl INode {
    ...
    fn get_size(&self, list: Option<Shared<Vec<usize>>>, target_size: usize) -> usize {
        let sz = if let Type::File { size } = self.node_type {
            ...
            self.children
                .borrow_mut()
                .iter()
                .map(|x| x.borrow().get_size(list.clone(), target_size))
                .sum::<usize>()
        };
...
fn main() {
    ...
    // Get the size of root first
    let root_size = root_saved.borrow().get_size(None, 0);

    println!("Total size of root = {}", root_size);

    // part2
    let unused_space = 70000000 - root_size;
    let target_size = 30000000 - unused_space;
    println!("The space that needs to be freed = {}", target_size);
    let _ = root_saved
        .borrow()
...
mod test {
    ...
    fn test_get_size() {
        ...
        assert!(root.borrow().get_size(None, 0) == 3800);
        assert!(game.borrow().get_size(None, 0) == 3500);
        assert!(csgo.borrow().get_size(None, 0) == 1500);
        assert!(music.borrow().get_size(None, 0) == 300);
    }
